...
import { getEncryptionSession, getUserSessionByConnectionId, newEncryptionSession, setEncryptionSession, } from "../services/session.js";
import { lobbyCommandMap } from "./lobbyCommands.js";
import { getServerLogger } from "rm-shared";
const log = getServerLogger();
export async function processEncryptedGameCommand(connectionId, message, socketCallback) {
    ...
    let encryptionSession = await getEncryptionSession(connectionId);
    ...
    const decryptedbytes = encryptionSession.gameDecipher.update(message.getDataAsBuffer());
    ...
    // Process the message
    let response = await processor(decryptedMessage.header.getId(), decryptedMessage.getDataAsBuffer());
    // Log the response
