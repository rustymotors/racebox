...
export async function processEncryptedGameCommand(
  ...
  log.info(
    `Encrypted response: ${encryptedResponse.length} bytes, ${getAsHex(encryptedResponse)}`,
  );
  ...
  log.info(
    `Response packet: ${responsePacket.header.getLength()} bytes, ${getAsHex(responsePacket.serialize())}`,
  );
